...
public class RisingEdgeLocalization extends Thread implements UltrasonicController {
	...
	public RisingEdgeLocalization(EV3LargeRegulatedMotor leftMotor, EV3LargeRegulatedMotor rightMotor,
			Odometer odometer) {
	...
	/**
	 * This method detects a rising edge, when the sensor gets away from the
	 * wall, and a falling edge, when the sensor gets closer to the wall. In
	 * order to do so, if the distance returned by the robot is bigger than a
	 * fixed distance, which is the horizontal constant, and we still didn't
	 * detect a rising edge, this means that the robot heading is moving away
	 * from a wall. The angle returned by the odometer is saved. Then, if the
	 * robot has already detected a rising edge, and the distance measured by
	 * the robot gets lower than the constant, this means that the robot heading
	 * is getting closer to a wall, and a falling edge is detected. The angle
	 * return by the odometer is then saved. Then, the method uses both angles
	 * to determine by how much the robot should turn to make it face north (0
	 * degrees)
	 */
