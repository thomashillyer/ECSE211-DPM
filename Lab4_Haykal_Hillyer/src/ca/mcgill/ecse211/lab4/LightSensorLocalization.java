...
public class LightSensorLocalization extends Thread {
	...
	public void run(){
		...
		// Adjust the robot position, before it starts to rotate to ensure that the light sensor will cross 4 black lines
		...
		//while rotating, get the angles which are used to correct the robot's position and orientation
		...
		System.out.println("x: " + odometer.getX() + " y: " + odometer.getY() + " theta: " +odometer.getTheta() * 180.0 / Math.PI );
	}
	
	/**
	 * This method calculates the robot's actual x and y position using the angle values determine earlier using trigonometry, and then
	 * determine the theta by which the odometer is off. Then, it updates the x, y and theta values of the odometer in order to fix them.
	 * Then, it uses the navigation travelTo method to travel to the inputted destination. 
	 * @param xdestination
	 * @param ydestination
	 */
	...
	/**
	 * while the robot is rotating on itself, get the measured values of the light sensor. A black line is detected
	 * when the derivative less than a threshold. When it is detected, a filter is added because sometimes, since the
	 * frequency of the value returned by the robot is high, the derivative can be less than the threshold for 2 or 3 
	 * consecutive returned values. This filter makes sure that we enter the if statement only the first time the derivative
	 * is less than threshold. Moreover, a line counter is added to determine which theta is return by the odometer (theta (x-), 
	 * theta (y+), theta (x+), and theta (y-) respectively).
	 */
	...
	/**
	 * This method adjusts the robot position before it starts rotating. It ensures that the light sensor,
	 * mounted on the back of the robot, runs over 4 black lines. It order to so, the robot, facing north
	 *  after the ultrasonic localization, moves forward until it detects a line, and then move backwards
	 *  1.5 times its center distance. This, ensures that the robot is close enough to the horizontal black
	 *  line facing the robot. Then, the method makes the robot turn 90 degrees and repeat the same procedure,
	 *  so it can be placed close enough to the vertical line. Now the robot it close enough to the first intersection.
	 */
	private void adjustRobotStartingPosition(){
		
		// This method gets the data from the light sensor when the robot is moving forward, and returns when a black line is detected
		detectBlackLine();
	...
	/**
	 * This method makes the robot move forward. While moving, get the value returned by the light sensor, compute
	 * the derivative, and if the value of the derivative (diff) is than the threshold, a black line is detected. Beep 
	 * and break out of the loop when the black line is detected.
	 */
