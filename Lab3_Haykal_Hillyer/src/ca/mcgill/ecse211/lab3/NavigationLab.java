...
public class NavigationLab {
	...
	public static void main(String[] args) {
		 ...
		 * resource // usSensor is the instance SensorModes usSensor = new
		 * EV3UltrasonicSensor(usPort); // usDistance provides samples from this
		 * instance SampleProvider usDistance = usSensor.getMode("Distance"); // usData
		 * is the buffer in which data are returned float[] usData = new
		 * float[usDistance.sampleSize()]; /* End lab 1 code
		 */
		...
		if (buttonChoice == Button.ID_LEFT) {
			...
			// USPoller uspoller = new USPoller(usDistance, usData, navigat, odometer,
			// navptone, leftMotor, rightMotor);
			// uspoller.start();
